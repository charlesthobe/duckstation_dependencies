...
if [ "$CROSSARCH" == "arm64" ]; then
  ...
  ARM_NEON_FLAGS="--target=arm-linux-gnueabihf -mfloat-abi=hard -mfpu=neon"
else
...
set(CMAKE_C_COMPILER_AR "/usr/bin/llvm-ar-${LLVM_VER}")
set(CMAKE_C_COMPILER_RANLIB "/usr/bin/llvm-ranlib-${LLVM_VER}")
...
set(CMAKE_CXX_COMPILER_AR "/usr/bin/llvm-ar-${LLVM_VER}")
set(CMAKE_CXX_COMPILER_RANLIB "/usr/bin/llvm-ranlib-${LLVM_VER}")
