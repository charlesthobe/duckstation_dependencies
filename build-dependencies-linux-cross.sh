...
if [ "$CROSSARCH" == "arm64" ]; then
  ...
  ARM_HF_FLAGS="--target=arm-linux-gnueabihf -mfloat-abi=hard -mfpu=vfpv3-d16"
else
...
set(CMAKE_C_FLAGS_INIT "-I${SYSROOTDIR}/usr/include/${CROSSTRIPLET} ${ARM_HF_FLAGS}")
...
set(CMAKE_CXX_FLAGS_INIT "-I${SYSROOTDIR}/usr/include/${CROSSTRIPLET} ${ARM_HF_FLAGS} -stdlib=libc++")
